...
const compress = (file) => {
  ...
  download.disabled = true; setStatus('处理中…')
  ...
  new Compressor(file, {
    ...
    error(error) { setStatus(`处理失败：${error.message}`, true) }
  })
}

quality.addEventListener('input', () => { qualityValue.value = quality.value })
fileInput.addEventListener('change', (event) => compress(event.target.files[0]))
...
zone.addEventListener('drop', (event) => { event.preventDefault(); delete zone.dataset.active; compress(event.dataTransfer.files[0]) })
...
document.querySelector('#image-reset').addEventListener('click', () => {
  ...
  before.hidden = true; after.hidden = true; download.disabled = true; fileInput.value = ''; setStatus('已清空。')
})
