...
不过，真实语言模型的 Tokenizer 并不是先像人一样理解句子含义，再决定应该按照哪个中文词语切分。Tokenizer在训练时会“阅读”大量语料数据，哪种组合出现的频率更高，他就更加倾向于把词划分成哪种组合。如果他训练的数据里球拍的出现频率比拍卖频次更高，那么就可能划分为球拍（这里只是最理想的情况，实际训练xxxgpt帮我补充）。
...
\]
（gpt，这里用实际的矩阵替换，类似矩阵右箭头矩阵右箭头矩阵这种，而不是抽象的说明）
